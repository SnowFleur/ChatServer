							...
							const char	*helper_executable/*=NULL*/)
{
...
#ifdef UNICODE
	const wchar_t* Wlogger_name=reinterpret_cast<const wchar_t*>(logger_name);
	const wchar_t* Whelper_executable=reinterpret_cast<const wchar_t*>(helper_executable);


	wcscpy(m_name,TEXT("\\\\.\\pipe\\"));

	if (!logger_name)
	{	// no name was give , create name based on the current address+time
		// (you can modify it to use PID , rand() ,...
		unsigned long now = GetTickCount();
		Wlogger_name = m_name+ wcslen(m_name);
		swprintf((wchar_t*)logger_name,TEXT("logger%d_%lu"),(int)this,now);
	}
	else
	{	// just use the given name
		wcscat(m_name,Wlogger_name);
	}
	...
	if (m_hPipe==INVALID_HANDLE_VALUE)
	{	// failure
		MessageBox(NULL,TEXT("CreateNamedPipe failed","ConsoleLogger failed"),NULL,MB_OK);
		return -1;
	}
...
#ifdef UNICODE
swprintf(cmdline,"%S %S",Whelper_executable,Wlogger_name);
#else
sprintf(cmdline,"%s %s",helper_executable,logger_name);
	...
	{	// on failure - try to get the path from the environment
		...
		if (path)
		{
			...
			bRet = CreateProcess(NULL,TEXT(cmdline),NULL,NULL,FALSE,CREATE_NEW_CONSOLE,NULL,NULL,&si,&pi);
		}
		if (!bRet)
		{
			MessageBox(NULL,TEXT("Helper executable not found","ConsoleLogger failed"),NULL,MB_OK);
			CloseHandle(m_hPipe);
			m_hPipe = INVALID_HANDLE_VALUE;
			return -1;
		}
	...
	if (!bConnected)
	{
		MessageBox(NULL,TEXT("ConnectNamedPipe failed","ConsoleLogger failed"),NULL,MB_OK);
	...
	{
		MessageBox(NULL,TEXT("WriteFile failed(1)","ConsoleLogger failed"),NULL,MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
